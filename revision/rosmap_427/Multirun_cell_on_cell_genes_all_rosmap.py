...
def run_single_split(exp_type, cell_type, split_index):
    ...
    if os.path.exists(rfe_cache_path):
        print(f"RFE cache found at {rfe_cache_path}, skipping RFECV/RFE computation.")
        selected_genes = pd.read_csv(rfe_cache_path)['selected_feature'].tolist()
        selected_genes = [g for g in selected_genes if g in train_matrix_filtered.columns]
        print(f"Loaded {len(selected_genes)} cached RFE features.")
    else:
