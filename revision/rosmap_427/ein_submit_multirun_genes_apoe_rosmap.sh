...
cell_types=()  # UNUSED for ROSMAP: Inh loads fine -> runs via the non-ein submit_multirun_genes_apoe. Only Ex/Oli need downsampling (exoli).
#  "Mic" "Opc" removing these cuz they were run add them back if you rerun this
num_splits=5
...
for ct in "${cell_types[@]}"; do
    for split in $(seq 1 $num_splits); do
        ...
        sbatch --account=patel ein_Multirun_cell_on_cell_genes_apoe_rosmap.sh "$experiment" "$ct" "$split"
        sleep 1
    done
done
