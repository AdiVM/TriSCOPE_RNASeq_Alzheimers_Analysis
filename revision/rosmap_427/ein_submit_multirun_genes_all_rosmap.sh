...
cell_types=()  # UNUSED for ROSMAP: Inh loads fine -> runs via the non-ein submit_multirun_genes_all. Only Ex/Oli need downsampling (exoli).
#  only for inh and ex are these jobs being run
num_splits=5
...
for ct in "${cell_types[@]}"; do
    for split in $(seq 1 $num_splits); do
        ...
        sbatch --account=patel ein_Multirun_cell_on_cell_genes_all_rosmap.sh "$experiment" "$ct" "$split"
        sleep 1
    done
done
