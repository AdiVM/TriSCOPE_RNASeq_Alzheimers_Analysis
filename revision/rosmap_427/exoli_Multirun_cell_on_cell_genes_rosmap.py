# Script is a copy of F1_Cell_on_cell_stable and is being used to train on just gene expression data
import argparse
import os
import pandas as pd
import numpy as np
...
log_dir_path = "/n/groups/patel/adithya/Alz_Outputs/Final_Outputs/Multirun_cell_on_cell_genes_rosmap427/"
...
def main():
    ...
    # Remove averaging here — do it in a post-processing script later
...
def run_single_split(exp_type, cell_type, split_index):
    ...
    log_message = f"Processing {exp_type} data with {cell_type} cells using full integration of gene and metadata features for split number {split_index}"
    ...
    with open(LOG_FILE_PATH, 'a') as log_file:
        log_file.write(f"Multirun on just gene expression data{cell_type}\n")

    # Load the data
    metadata = pd.read_parquet('/n/groups/patel/adithya/ROSMAP427_Outputs/ROSMAP427_CellMetadata.parquet').set_index('TAG')

    # metadata = pd.read_parquet('/home/adm808/New_CellMetadataSyn1848517.parquet')
    #metadata = pd.read_csv('/home/adm808/UpdatedCellMetadataSyn16780177.csv', low_memory=False)
    ...
    EXOLI_CAP = {"Ex": 400, "Inh": 400, "Oli": 400}  # 750/donor still OOMed apoe/all at AutoML (~240k cells x ~11k features). 400/donor (~128k Ex) + filtered read + del gene_matrix fits 249G. Lower than SEA-AD's 1000 purely because ROSMAP-427 has far more nuclei/donor; the cap is a memory/compute parameter. Ast/Mic/Opc full-load via non-ein.
    if cell_type in EXOLI_CAP:
        n_cap = EXOLI_CAP[cell_type]
        train_metadata = (
            train_metadata
            .groupby('Donor ID', group_keys=False)
            .apply(lambda x: x.sample(n=min(len(x), n_cap), random_state=split_index))
        )
    ...
    # Memory-bounded read: load ONLY the donor-subsampled cells (selected above),
    # part-by-part, so the full cell-type matrix (up to ~140GB for Ex) is never
    # materialized. Equivalent to a full read + .loc filter, just OOM-safe.
    import glob as _glob
    import pyarrow as _pa
    import pyarrow.parquet as _pq
    import pyarrow.compute as _pc
    _needed = _pa.array(train_metadata.index.union(test_metadata.index).tolist())
    _parts = sorted(_glob.glob(f'/n/groups/patel/adithya/ROSMAP427_Outputs/ROSMAP427_Matrix_{cell_type}/*.parquet'))
    _tbls = []
    for _pth in _parts:
        _tb = _pq.read_table(_pth)
        _tb = _tb.filter(_pc.is_in(_tb['TAG'], value_set=_needed))
        if _tb.num_rows:
            _tbls.append(_tb)
    gene_matrix = _pa.concat_tables(_tbls).to_pandas().set_index('TAG')
    ...
    del gene_matrix  # not used after the split; free ~50GB before RFECV to avoid OOM on Ex/Oli
    ...
    #     train_matrix_filtered = train_matrix
    #     test_matrix_filtered = test_matrix
    # else:
    ...
    # if cell_type in ["Ex", "Inh"]:
    #     optimal_feature_count = min(rfecv.n_features_, 1000)
    # else:
    ...
    train_data = train_matrix_filtered.merge(
        train_metadata[['Donor ID', 'Sex', 'broad_cell_type', 'alzheimers_or_control', 'Age at Death', 'PMI']],
        left_index=True,
        right_index=True,
        how='inner'
    )
    ...
    test_data = test_matrix_filtered.merge(
        test_metadata[['Donor ID', 'Sex', 'broad_cell_type', 'alzheimers_or_control', 'Age at Death', 'PMI']],
        left_index=True,
        right_index=True,
        how='inner'
    )
    ...
    # Age of death variable is already a float


    # Dropping columns from the dataset
    # Dropping columns from the dataset
    metadata_cols = ['DonorID', 'Sex', 'broad_cell_type', 'alzheimers_or_control', 'AgeatDeath', 'PMI']
    import re

    apoe_cols_cleaned = [re.sub(r'[^A-Za-z0-9_]+', '', col) for col in apoe_genotype_columns]
    all_metadata_cols = metadata_cols + apoe_cols_cleaned

    # Ensure all present columns are dropped
    cols_in_X_train = set(X_train.columns)
    # Take the intersection — i.e., only the metadata columns that are actually in X_train
    drop_these = list(set(all_metadata_cols) & cols_in_X_train)


    X_train = X_train.drop(columns=drop_these)
    X_test = X_test.drop(columns=drop_these)
    ...
    with open(LOG_FILE_PATH, 'a') as log_file:
        log_file.write(f"Reached classifier for fixed Opc and Ex, this is genes only: {cell_type}\n")
    ...
    with open(LOG_FILE_PATH, 'a') as log_file:
            log_file.write(f"Finished genes only for cell on cell on the fix for Ex and Opc:{cell_type}\n")
