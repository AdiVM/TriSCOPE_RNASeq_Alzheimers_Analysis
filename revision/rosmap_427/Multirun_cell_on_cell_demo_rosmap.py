...
log_dir_path = "/n/groups/patel/adithya/Alz_Outputs/Final_Outputs/Multirun_cell_on_cell_demographics_rosmap427/"
...
def run_single_split(exp_type, cell_type, split_index):
    ...
    log_message = f"Processing {exp_type} data with {cell_type} cells using only metadata features for the seaad dataset"
    ...
    with open(LOG_FILE_PATH, 'a') as log_file:
        log_file.write(f"Multirun cell on cell training using just demographics: {cell_type} \n")
    ...
    train_data = train_metadata[['Donor ID', 'Sex', 'broad_cell_type', 'alzheimers_or_control', 'Age at Death', 'Education', 'PMI'] + apoe_genotype_columns].copy()
    test_data = test_metadata[['Donor ID', 'Sex', 'broad_cell_type', 'alzheimers_or_control', 'Age at Death', 'Education', 'PMI'] + apoe_genotype_columns].copy()
    ...
    # metadata_features = ['DonorID', 'Sex', 'AgeAtDeath', 'PMI'] + [col for col in train_data.columns if col.startswith("APOEGenotype")]
    metadata_features = ['DonorID', 'Sex', 'AgeatDeath', 'Education', 'PMI'] + [col for col in train_data.columns if col.startswith("APOEGenotype")]
