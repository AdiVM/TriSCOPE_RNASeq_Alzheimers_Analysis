...
out_dir <- "/Users/adithyamadduri/Desktop/Projects/Patel_Lab/Revision/NPvsCAD/Mathys_New_Data_DEG_results"
...
for (celltype in cell_types) {
  ...
  ## --- Restrict to neuropathology-positive individuals only ---
  meta_mic <- meta_mic %>%
    filter(ceradsc %in% c(1, 2))
  
  cat(
    "Cells after restricting to CERAD-positive (1–2):",
    nrow(meta_mic), "\n"
  )
  ...
  ## Clinical AD status among CERAD-positive individuals
  ## 1 = clinically diagnosed AD (dcfdx_lv 4–5)
  ## 0 = neuropathology-positive but no clinical diagnosis
  meta_mic$AD <- as.factor(
    ifelse(meta_mic$dcfdx_lv %in% c(4, 5), 1, 0)
  )
