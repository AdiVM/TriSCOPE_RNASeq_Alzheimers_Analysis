...
cell_types=("Oli" "Ast" "Mic" "Opc")
#  "Inh" "Ex" removing these cuz they are run using the ein_submit files
num_splits=5
...
for ct in "${cell_types[@]}"; do
    for split in $(seq 1 $num_splits); do
        ...
        sbatch --account=patel Multirun_cell_on_cell_genes_seaad.sh "$experiment" "$ct" "$split"
        sleep 1
    done
done
